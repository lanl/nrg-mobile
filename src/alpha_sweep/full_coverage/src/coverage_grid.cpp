...
void CoverageGrid::print(std::string name)
{
  ...
  cv::cvtColor(typeMat, typeMat, CV_RGB2BGR);
  cv::cvtColor(nbsMat, nbsMat, CV_RGB2BGR);
  cv::cvtColor(reachMat, reachMat, CV_RGB2BGR);
  cv::cvtColor(visMat, visMat, CV_RGB2BGR);
  try
  {
    std::vector<int> comp_p;
    comp_p.push_back(CV_IMWRITE_PNG_COMPRESSION);
    comp_p.push_back(0);
